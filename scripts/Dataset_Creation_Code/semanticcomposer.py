...
"""
Semantic Duplicate-Object Composer (same background, multiple objects)
=====================================================================

This version adds flexible path mapping so it works with Falak's dataset:
  ORIGINAL_ROOT = /home/falak/UAV_dataset/combined_uav
  ├─ input/
  │   ├─ images/               (flat RGB images)
  │   ├─ train/<scene>/{infrared,visible}/*.jpg
  │   ├─ val/<scene>/{infrared,visible}/*.jpg
  │   └─ test/<scene>/{infrared,visible}/*.jpg
  └─ labels/
      ├─ labels/               (flat RGB masks)
      ├─ train/<scene>/{infrared,visible}/*_mask.png
      ├─ val/<scene>/{infrared,visible}/*_mask.png
      └─ test/<scene>/{infrared,visible}/*_mask.png

How the mapping works now:
- Use --images_dirname to point to "input" (default was "UAV-Image").
- Use --masks_dirname  to point to "labels" (default was "UAV-Mask").
- If a split name on the images side differs from the masks side, add an alias via
  --mask_split_aliases images:labels  (so split "images" uses masks split "labels").

Examples:
1) Process train/val/test (visible/infrared scenes):
python3 semanticcomposer.py \
  --original_root /home/falak/UAV_dataset/combined_uav \
...
2) Additionally process the flat RGB folder (input/images ↔ labels/labels):
python3 semanticcomposer.py \
  --original_root /home/falak/UAV_dataset/combined_uav \
  --out_root /home/falak/UAV_dataset/combined_uav_multi \
  --images_dirname input --masks_dirname labels \
...
Notes:
- Works whether images are directly in the split dir (flat) or nested (scene/visible).
- Robust mask matching:
    * {base}.png, {base}_mask.png, {base}-mask.png
    * {base}_label.png, {base}_gt.png, {base}_seg.png
    * any of the above with .png/.bmp/.tif/.tiff/.jpg/.jpeg
    * Fallback: finds by the numeric token shared in the filename (e.g., 10904)
- Keeps the original object; adds (N-1) extra copies with scale/rotate/flip.
- Output mirrors input structure under OUT_ROOT with *_multiK in filenames.
"""
