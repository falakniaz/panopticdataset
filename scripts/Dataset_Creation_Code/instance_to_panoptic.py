...
"""
Instance → COCO‑Panoptic Converter for Falak's UAV datasets
==========================================================

This version plugs into your folder choices and adds an optional background
stuff segment so PanopticFPN can train even if you only have a single "thing"
class (your UAV object) and no explicit stuff masks.

Inputs (configurable via flags):
  IMAGE_ROOT/<images_dirname>/<split>/<...>/*.{jpg,png,...}
  INSTANCE_ROOT/<instance_masks_dirname>/<split>/<...>/*.png  (uint16 instance ids; 0=bg)

Key flags:
  --mask_split_aliases images:labels   # use when IMG split name differs from MASK split name
  --add_background_stuff               # add a single background stuff segment per image
  --background_category_id 0           # id for background stuff
  --background_category_name background

Examples
--------
A) Current flat RGB split (images ↔ labels):
python3 panoptic_converter_falak.py \
  --image_root      /home/falak/Full_revise_dataset \
  --instance_root   /home/falak/Full_revise_dataset/Panoptic_dataset/instance \
  --out_root        /home/falak/Full_revise_dataset/Panoptic_dataset/panoptic \
  ...
  --add_background_stuff

B) If you later build train/val/test splits:
python3 panoptic_converter_falak.py \
  --image_root      /home/falak/Full_revise_dataset \
  --instance_root   /home/falak/Full_revise_dataset/Panoptic_dataset/instance \
  --out_root        /home/falak/Full_revise_dataset/Panoptic_dataset/panoptic \
...
Output
------
  OUT_ROOT/annotations/panoptic.json
  OUT_ROOT/panoptic_<split>/**.png      (RGB-encoded segment ids)

Notes
-----
- If your instance masks are *binary*, you can derive components with
  --from_binary_components. Otherwise keep them as uint16 instances.
- id scheme: segment_id = image_id*id_multiplier + local_instance_id
"""
